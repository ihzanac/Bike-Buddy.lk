...
const ROUTE = '/api/upload/part-image'
...
function createHandler(publicDir: string): Connect.NextHandleFunction {
  return (req, res, next) => {
    ...
    bb.on('file', (name, file, info) => {
      ...
      const fname = `part-${Date.now()}-${randomUUID().slice(0, 8)}${ext}`
      const underPublic = join('uploads', 'parts', sub, fname)
...
export function vitePluginPartImageUpload(): Plugin {
  return {
    name: 'part-image-upload',
